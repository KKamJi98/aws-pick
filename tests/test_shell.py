...
@patch("aws_profile_switcher.shell.shutil.copy2")
@patch("aws_profile_switcher.shell.datetime")
...
@patch("aws_profile_switcher.shell.get_zshrc_path")
@patch("aws_profile_switcher.shell.backup_zshrc")
@patch("builtins.open", new_callable=mock_open, read_data="# Some content\nexport AWS_PROFILE=\"old-profile\"\n")
def test_update_aws_profile_existing(mock_file, mock_backup, mock_get_path):
...
@patch("aws_profile_switcher.shell.backup_zshrc")
@patch("builtins.open", new_callable=mock_open, read_data="# Some content\n")
def test_update_aws_profile_new(mock_file, mock_backup, mock_get_path):
    ...
    written_content = "".join(call_args[0][0] for call_args in handle.write.call_args_list)
    assert 'export AWS_PROFILE="new-profile"' in written_content
    assert '# Added by AWS Profile Switcher' in written_content


@patch("aws_profile_switcher.shell.get_zshrc_path")
@patch("builtins.open", new_callable=mock_open, read_data='export AWS_PROFILE="same-profile"\n')
def test_update_aws_profile_no_change(mock_file, mock_get_path):
